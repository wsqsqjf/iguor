...
<div class="input-append">
	...
	<input id="${id}Name" name="${labelName}" readonly="readonly" type="text" value="${labelValue}" maxlength="50"
		class="${cssClass}" style="${cssStyle}"/><a id="${id}Button" href="javascript:" class="btn ${disabled}" style="line-height: 36px;height: 36px;padding: 2px 3px 0 3px;">&nbsp;<i class="icon-search"></i>&nbsp;</a>&nbsp;&nbsp;
</div>
	...
	$("#${id}Button").click(function(){
		...
		top.$.jBox.open("iframe:${ctx}/tag/treeselect?url="+encodeURIComponent("${url}")+"&module=${module}&checked=${checked}&extId=${extId}&selectIds="+$("#${id}Id").val(), "选择${title}", 300, 420, {
			buttons:{"确定":"ok", ${allowClear ? "\"清除\" : \"clear\", " : ""}"关闭":true}, submit:function(v, h, f){
				...
				$(".jbox-content", top.document).css("position","absolute");
				$(".jbox-content", top.document).css("left","10px");
				$(".jbox-title-panel", top.document).css("height","20px");
				$(".jbox-button-panel", top.document).css("height","26px");
			}
		});
	});
</script>